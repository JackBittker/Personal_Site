...
var pointCount = 500;
var str = "JACK BITTKER";
var fontStr = "bold 50pt italic, arial,sans serif";

ctx.font = fontStr;
ctx.textAlign = "center";
c.width = ctx.measureText(str).width;
c.height = 50; // Set to font size
...
function init() {
    ...
    ctx.textAlign = "left";
    ctx.fillStyle = "#fff";
    ctx.fillText(str, .5, c.height / 2 + (c.height / 2));
    ctx.closePath();
...
function addPoint() {
    ...
    var p = new point(spawn[0], spawn[1], Math.floor(Math.random() * 2 - 1), Math.floor(Math.random() * 2 - 2));
    points.push(p);
}
